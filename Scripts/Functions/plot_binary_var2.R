...
plot_binary_var2 <- function(data, variable_name, title_name, x_var= "clad4", facet_var = "clad4", ncol = 1) {
  ...
  long_var <- data %>%
    dplyr::select(Revenue_K, clad4, rip, ateco_1,year, all_of(variable_name)) %>%
    pivot_longer(cols = -c(Revenue_K, clad4, rip, ateco_1, year), 
